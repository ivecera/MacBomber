...
- (void)setupWorkingDirectory:(BOOL)shouldChdir
{
	...
	*c++ = '\0'; /* cut off last part (binary name) */
	...
	if (shouldChdir) {
		assert(chdir(parentdir) ==
		       0); /* chdir to the binary app's parent */
		assert(chdir("../../../") ==
		       0); /* chdir to the .app's parent */
	}
}
...
- (void)fixMenu:(NSMenu *)aMenu withAppName:(NSString *)appName
{
	...
	if (aRange.length != 0)
		[aMenu setTitle:[[aMenu title] stringByReplacingRange:aRange
								 with:appName]];
...
- (void)applicationDidFinishLaunching:(NSNotification *)note
{
	...
	[self fixMenu:[NSApp mainMenu]
		withAppName:[[NSProcessInfo processInfo] processName]];
#endif
