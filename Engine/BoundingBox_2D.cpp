...
bool BoundingBox_2D::doesCollideWith(BoundingBox_2D *pBBox)
{
				 ...
				 const glm::vec3 &vVelocityVector, float &time)
{
	...
	float tEnterX =
		66; //time when pBBox will enter (collide) this boundingbox on x axis
	float tLeaveX =
		66; //time when pBBox will leave this boundingbox on x axis
	float tEnterZ =
		66; //time when pBBox will enter (collide) this boundingbox on z axis
	float tLeaveZ =
		66; //time when pBBox will leave this boundingbox on z axis

	//check on x-Axis  (if necessary)
	if (vVelocityVector.x != 0) {
		tEnterX = (m_vMin.x - pBBox->m_vMax.x) /
			  (Application::m_fReciprocalFPS * vVelocityVector.x);
		tLeaveX = (m_vMax.x - pBBox->m_vMin.x) /
			  (Application::m_fReciprocalFPS * vVelocityVector.x);
	...
	if (vVelocityVector.z != 0) {
		tEnterZ = (m_vMin.z - pBBox->m_vMax.z) /
			  (Application::m_fReciprocalFPS * vVelocityVector.z);
		tLeaveZ = (m_vMax.z - pBBox->m_vMin.z) /
			  (Application::m_fReciprocalFPS * vVelocityVector.z);
